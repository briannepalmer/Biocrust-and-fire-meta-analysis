...
```{R load data}
cover <- read.csv("C:/Users/Brianne/OneDrive - San Diego State University (SDSU.EDU)/R WorkBooks/MetaAnalysis/data/FireMA_June2020.csv") # control is unburned
# make TSF a number
...
early <- cover %>% filter(crustType %in% c("cyanobacteria", "algae"))
early$Succession <- rep("early", n=25)
late <- cover %>% filter(crustType %in% c("lichen", "bryophyte"))
late$Succession <- rep("late", n = 184)
unknown <- cover %>% filter(crustType %in% c("unknown", "mixed"))
unknown$Succession <- rep("unknown", n = 28)

early.late <- full_join(early, late)
cover <- full_join(early.late, unknown)
...
brt.control.cover <- gbm.step(data=brt.cover.data, gbm.x = c(4:17, 25), gbm.y = 21, family = "gaussian", tree.complexity = 5,learning.rate = 0.01, bag.fraction = 0.5)
...
burn.contributions$var <- c("Biocrust Type", "Location", "TSF", "Temperature", "Radiation", "Fire Regime", "Aridity", "Wind Speed", "Precipitation", "Water Vapor", "Plot Size", "Fire Type")

pdf(file = "C:/Users/Brianne/OneDrive - San Diego State University (SDSU.EDU)/R WorkBooks/MetaAnalysis/figures/burnBRT.pdf")
...
pdf(file = "figures/controlBRT.pdf")
...
data_es <- escalc(n1i = Burn.n, n2i = Cont.n, m1i = Burn.Cover, m2i = Cont.Cover, sd1i = Burn.se, sd2i = Cont.se, data = cover, measure = "ROM", "SMD")
```
...
ggplot(data = data_es, aes(y = yi, x = ))
